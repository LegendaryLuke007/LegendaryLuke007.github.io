...
> There are reasonable security concerns in this setup with bringing traffic from the internet through your network. Make sure to have proper firewall configurations and don't allow the Home Server to just be fully open. Isolating the *Indexer* and the *Server* to a VM on the Home Server is an ideal hardening method.
{: .prompt-tip }
